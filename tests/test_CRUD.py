...
class TestLoad:
    ...
    def test_load_with_songs(self, library_with_songs):
        ...
        assert all(isinstance(t, Title) for t in result), (
            "Alle Elemente sollten Title-Objekte sein"
        )
    ...
    def test_load_nonexistent_file(self, tmp_path):
        ...
        assert lib.titles == [], (
            "Sollte leere Liste bei nicht existierender Datei erstellen"
        )
...
class TestAddTitle:
    ...
    def test_add_title_preserves_existing_id(self, empty_library):
        ...
        title = Title(
            id=24, name="Test", artist="Artist", album="Album", year=2000, genre="Rock"
        )
        result = empty_library.add_title(title)
...
class TestUpdateTitle:
    ...
    def test_update_multiple_fields(self, library_with_songs):
        ...
        result = library_with_songs.update_title(
            1, name="New Name", artist="New Artist", year=2000
        )

        assert result["name"] == "New Name"
        assert result["artist"] == "New Artist"
        assert result["year"] == 2000
        assert result["album"] == "Imagine", (
            "Nicht-aktualisierte Felder sollten gleich bleiben"
        )

    def test_update_all_fields(self, library_with_songs):
        """Alle Felder aktualisieren."""
        result = library_with_songs.update_title(
            1, name="New", artist="Artist", album="Album", year=2024, genre="Pop"
        )
...
class TestSearch:
    ...
    def test_search_with_nonexistent_field(self, library_with_songs):
        ...
        assert results == [], (
            "Sollte leere Liste bei nicht existierendem Feld zurückgeben"
        )
